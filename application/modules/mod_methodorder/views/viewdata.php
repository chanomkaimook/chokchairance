    ...
    <script>
        ...
        $(function() {
            ...
            $(document).on('click', '#editform', function(e) {
                ...
                window.location.replace('editdata?id=' + billID);
            })
            ...
            function formDataInsert(bill) {
                console.log(bill)
                let usercreate = bill.data_user_starts;
                let datecreate = toThaiDateTimeString(new Date(bill.data_date_starts), 'datetime');

                let userupdate;
                let dateupdate;
                if (bill.data_date_update && bill.data_date_update != '0000-00-00 00:00:00') {
                    dateupdate = toThaiDateTimeString(new Date(bill.data_date_update), 'datetime');
                }

                //  set style div
                frm.find('.data_name').text(bill.data_name);
                frm.find('.data_bu_name').text(bill.data_delivery_name);
                frm.find('.bill_staffcreate').text(bill.data_user_starts);
                frm.find('.bill_datecreate').text(datecreate);
                frm.find('.bill_staffedit').text(bill.data_user_update);
            ...
            $(document).on('click', '#btn-cancel', function() {
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        let url = '../../api/methodorder/delete/' + billID;
                        let options = {
                            headers: {
                        ...
                        fetch(url, options)
                            ...
                            .then((resp) => {
                                ...
                                if (resp.error_code) {
                                    swal.fire('ข้อมูลผิดพลาด', resp.data, 'warning');
                                    $('.loading').remove();
                                ...
                                window.location.replace('methodorder');
                            })
                            .catch(error => {
