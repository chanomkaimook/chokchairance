...
// exit;
		...
		echo "<pre>";
		echo mb_convert_encoding($spreadSheetAry[1]['A'], 'UTF-16LE', 'UTF-8');
...
function cleanData($str)
{
	...
	$str = mb_convert_encoding($str, 'UTF-16LE', 'UTF-8');
				...
				foreach ($spreadSheetAry[$i] as $key => $value) {
					...
					if ($i > 6) {
						...
						if ($key == 0 && $value) {
							...
							/* $newDate = mb_convert_encoding($datainsert[$spreadSheetAry[$i][$key]], 'UTF-8', mb_detect_encoding($datainsert[$spreadSheetAry[$i][$key]]));
								$array['bill']['date'] = $newDate; */
							$start_bill = 1;
						...
						if ($start_bill) {
							...
							//	หาชื่อสินค้า
							if ($key == 5 && $value) {
								// $array_item[$item_no]['name'] = iconv('ASCII','UTF-8//ignore',$datainsert[$spreadSheetAry[$i][$key]]);
								// $array_item[$item_no]['name'] = iconv('UTF-8',false,$datainsert[$spreadSheetAry[$i][$key]]);
								// $array_item[$item_no]['name'] = iconv('TIS-620','UTF-8',$datainsert[$spreadSheetAry[$i][$key]]);
								// $array_item[$item_no]['name'] = mb_convert_encoding($datainsert[$spreadSheetAry[$i][$key]], 'utf-8', mb_detect_encoding($datainsert[$spreadSheetAry[$i][$key]]));
								// $array_item[$item_no]['name'] = $datainsert[$spreadSheetAry[$i][$key]];
								// $array_item[$item_no]['name'] = iconv('UTF-8', 'TIS-620', mb_convert_encoding($datainsert[$spreadSheetAry[$i][$key]], "UTF-8"));
								// $array_item[$item_no]['name'] = mb_convert_encoding($datainsert[$spreadSheetAry[$i][$key]], "windows-1252");
								// $array_item[$item_no]['name'] = cleanData($datainsert[$spreadSheetAry[$i][$key]]);
								// $t = chr(255) . chr(254) . $datainsert[$spreadSheetAry[$i][$key]];
								$array_item[$item_no]['name'] = cleanData($datainsert[$spreadSheetAry[$i][$key]]);
							...
							//	หาหน่วยสินค้า
							if ($key == 8 && $value) {
								// $array_item[$item_no]['unit'] = iconv('UTF-8', 'TIS-620', mb_convert_encoding($datainsert[$spreadSheetAry[$i][$key]], "UTF-8"));
								// $array_item[$item_no]['unit'] = mb_convert_encoding($datainsert[$spreadSheetAry[$i][$key]], 'UTF-16LE', 'UTF-8');
								$array_item[$item_no]['unit'] = iconv('TIS-620', 'UTF-16LE',$datainsert[$spreadSheetAry[$i][$key]]);
							}
					...
					/* if($array_error[$i][$key]){
							$array_item[$item_no] = array(); //1008428618
						} */
		...
		echo $chain."______<br>"; */
		echo "<pre>";
		
		/* $str[] = mb_convert_encoding($text, 'ISO-8859-15', 'UTF-8');
		$str[] = mb_convert_encoding($text, 'UTF-8', mb_detect_encoding($text));
		$str[] = mb_convert_encoding($text, "WINDOWS-1251");
		$str[] = mb_convert_encoding($text, "UTF-8");
		// $str[] = html_entity_decode($text,ENT_QUOTES,'UTF-8');
		// $str[] = html_entity_decode($text,ENT_QUOTES,'TIS-620');
		$str[] = iconv('TIS-620', 'UTF-8//ignore', $text);
		$str[] = iconv('UTF-8', 'TIS-620//ignore', $text);
		$str[] = iconv('UTF-8', 'ASCII//TRANSLIT', $text);
		$str[] = iconv('CP1252', 'UTF-16', $text);
		$str[] = iconv('CP1252', 'UTF-16BE', $text);
		$str[] = iconv('UTF-8', 'CP1251//TRANSLIT//IGNORE', $text);
